...
const ChatContainer: React.FC = () => {
    ...
    const [messages, setMessages] = useState<ChatTypes[]>([{
        role: "bot",
        message: "Salut… Qu'est-ce qui se passe ? Raconte-moi en quelques mots, je t'écoute."
    }]);
    ...
    useEffect(() => {
        if (messageCount >= MAX_USER_MESSAGES) {
            const emotionParam = detectedEmotion ? `emotion=${encodeURIComponent(detectedEmotion)}` : '';
    ...
    const detectEmotion = useMemo(() => {
        ...
        return (text: string): Emotion | null => {
            for (const [key, regex] of Object.entries(patterns) as [Emotion, RegExp][]) {
    ...
    const buildContext = useMemo(() => {
        return () => {
            let base = "Tu es un assistant conversationnel spécialisé dans l'intelligence émotionnelle.";
            if (emotionStep === "integration" && detectedEmotion) {
                const { labelFr } = EMOTIONS_AVAILABLE.find(e => e.constant === detectedEmotion)!;
    ...
    const sendUserMessage = async (message: string) => {
        ...
        if (newEmotion) {
            ...
            setEmotionStep(prev => {
                switch (prev) {
                    case "neutral": return "detection";
                    case "detection": return "confirmation";
                    case "confirmation": return "integration";
                    default: return prev;
                }
            });
        }
        ...
        setIsLoading(true);
        setMessages(prev => [...prev, { message: "Je réfléchis...", role: "bot" }]);
        ...
        try {
            ...
            setMessages(prev => {
                ...
                copy[last] = {
                    ...
                    role: "bot",
                    emotion: emotionStep === "integration" && detectedEmotion
                        ? EMOTIONS_AVAILABLE.find(e => e.constant === detectedEmotion)!.emoji
                        : undefined
                };
                return copy;
            });
            ...
            setMessages(prev => {
                const copy = [...prev];
                copy[copy.length - 1] = { message: "Une erreur est survenue.", role: "bot" };
                return copy;
            });
        } finally {
            setIsLoading(false);
    ...
    return (
        <div className="mx-auto flex w-full max-w-xl flex-col justify-between p-4">
            <div className="hide-scrollbar mt-5 space-y-4 overflow-y-scroll">
                {messages.map((m, i) => (
                    <ChatContent key={i} isUser={m.role === "user"} message={m.message} emotion={m.emotion} />
                ))}
                {isLoading && <ChatContent isUser={false} message="..." />}
            </div>
            <ChatInput sendMessage={sendUserMessage} disabled={isLoading || messageCount >= MAX_USER_MESSAGES} />
        </div>
    );
};
